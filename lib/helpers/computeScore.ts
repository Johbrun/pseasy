...
export default function computeScore(
    questions: QuizzQuestionFull[],
    answers: Array<QuizzAnswerCreation | undefined>
) {
    let score = 0;
    questions.forEach((question) => {
        ...
        if (answer) {
            console.log(
                question,
                answer,
                Boolean(answer?.answer1Choice),
                Boolean(question.answer1IsOk)
            );
            if (
                Boolean(answer?.answer1Choice) ===
                    Boolean(question.answer1IsOk) &&
                Boolean(answer?.answer2Choice) ===
                    Boolean(question.answer2IsOk) &&
